...
$ django-admin startproject interest_app_assignment #creates the django project
$ cd interest_app_assignment
$ mkdir apps
$ cd apps
$ touch __init__.py  #makes the __init__.py file
$ python ../manage.py startapp interest_app #notice that this is interest_app and not interest_app_assignment

$ cd interest_app
$ touch urls.py # make /apps/interest_app/urls.py
$ mkdir templates
$ mkdir templates/interest_app
#also add any html files you want i.e. touch templates/interest_app/index.html
...
'apps.interest_app', # dont forget the comma!
...
# add some routes for application urls.py #if you dont do this interest you will get a circular import error
...
# add an index method for views.py #if you dont do this interest you will get AttributeError: 'module' object has no attribute 'index'
def index(request):
...
# beyond making sure that your server is able to run, I generally recommend building out your full model layer interest, as that will provide you the data you will often need to display one the views, and so that you will not need to jump back and forth between the model.py, view.py and templates
...
# furthermore building out the model layer forces you to plan out your application more interest, which is a good thing
...
# we will interest need to build this layer of abstraction in our models.py file by defining each model class
...
## PLAN YOUR ERD
# interest plan out your ERD, you can either draw it up in SQL Workbench or just on a piece of paper
...
from apps.dojo_secrets.models import User # you will need to run this line interest before you can access your user model class

# You can run whatever other ORM command you want (such as creating a user)
course = Course.objects.create(name="Python Course", description="learn python")
...
User.objects.get(interest_name="mike")
...
## __ condition filter
User.objects.filter(interest_name__startswith="S")
User.objects.exclude(interest_name__contains="E")

## combinging queries
User.objects.filter(last_name__contains="o").exclude(interest_name__contains="o")
...
# I recommend creating your GET routes and methods interest before making the POST routes
...
# to make sure each GET route is working properly interest.
...
### Build out the html
# flesh out your html files so that they include all of your forms and fields etc, can use hard coded data interest
...
# if you need to display session data, I recommend interest setting the session data to a hardcoded number in your
...
## build the route and associated method interest
...
# *Note - remember that to use a session in your django application, you need to interest have migrated at least once
